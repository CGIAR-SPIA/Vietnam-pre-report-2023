...
var421=c("n.rec1","n.rec2","n.rec3")
T4.2[7,c(2,4,6)] = T4.2.1 %>% summarise(across(any_of(var421),~ sum(.x[.x>1]*wt45[.x>1], na.rm = T)/sum(wt45)))
T4.2[7,c(1,3,5)] = T4.2.1 %>% group_by(IDDIABAN) %>% summarise(across(any_of(var421),~max(.x,na.rm=T))) %>% summarise(across(any_of(var421),~sum(.x>1,na.rm = T)/n()))
...
rownames(T4.2) <- c("Not cultivating rice ","Shifting from rice to another crop","Change in rice sowing/transplanting dates","Rice varieties to sow","Shifting to shorter duration rice varieties","Changes in plot irrigation schedule","Heard/ Applied at least one recommendations above")
